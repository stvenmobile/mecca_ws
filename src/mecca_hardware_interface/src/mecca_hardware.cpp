...
hardware_interface::return_type MeccaHardware::write(const rclcpp::Time &, const rclcpp::Duration &)
{
  ...
  int cmd_x = static_cast<int>(vx * CMD_SCALE_FACTOR) * -1; // Inverted: Forward Stick -> Forward Robot
  int cmd_y = static_cast<int>(vy * CMD_SCALE_FACTOR);
  int cmd_rot = static_cast<int>(omega * CMD_SCALE_FACTOR * 0.5) * -1; // Inverted: Fixes M1/M2/M3/M4 wiring swap
